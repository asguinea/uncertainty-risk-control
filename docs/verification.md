...
3. The separate synthetic example, GoEmotions, HumAID and TweetEval Sentiment final-calibration replays, using the source archive's configurations and evidence. HumAID adds 16 final tests and 1,202 warm-up aggregate records; TweetEval Sentiment adds 35 final tests and 2,804 warm-up records. All three run before plotting dependencies are installed. Their combined inventory is 86 final tests and 6,810 aggregate records.
4. Aggregate table regeneration, compared with the tracked report after normalizing platform line endings.
5. All seven figures (two per study plus the study map) regenerated after installing the optional plotting dependencies from a hash-locked requirements export. All four numerical/provenance JSON inputs must match exactly, excluding only recorded environment metadata. Image-byte identity is not required across platforms. The new study figure JSON contains verified evaluation counts and warm-up aggregates. Unplotted final-test p-values and confidence-bound diagnostics remain in the complete replay receipt, checked with the existing numerical tolerances; they are not figure inputs.
...
The artifact upload is an explicit list: distribution and execution receipts, numerical verification, synthetic, GoEmotions, HumAID and TweetEval Sentiment replay JSON, tables, and generated figure files. Dependency-install logs, environment directories, raw source downloads, and built distribution archives are not uploaded as CI artifacts. The checked artifacts are retained for 14 days; a release should separately preserve its reviewed evidence and verification record. The release asset `verification-v0.2.0.json` records the actual CI run, exact commit, platform receipts and reviewed artifact hashes; the workflow definition alone is not evidence of a successful run.
